            ...
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    if( $Validasi['msg'] == "sukses"){
                        $File = $Validasi['pesan'];
                        $status = "pkl";
                        $sql = "INSERT INTO ims_sk_pengangkatan SET NoKtp = :NoKtp, KodeBranch = :KodeBranch, KodeCabang = :KodeCabang, KodeDivisi = :KodeDivisi, KodeSubDivisi = :KodeSubDivisi, KodeSeksi = :KodeSeksi, TanggalMulai = :TanggalMulai, NoDokumen = :NoDokumen,`File` = :Files,  Keterangan = :Keterangan, Kategori = :Kategori,  TglCreate = :TglCreate,  UserId = :UserId,`Status` = :Status";
                        ...
                        $exc->bindParam('Status', $pkl, PDO::PARAM_STR);
                        $exc->execute();
                    ...
                    $status = "pkl";
                    $sql = "INSERT INTO ims_sk_pengangkatan SET NoKtp = :NoKtp, KodeBranch = :KodeBranch, KodeCabang = :KodeCabang, KodeDivisi = :KodeDivisi, KodeSubDivisi = :KodeSubDivisi, KodeSeksi = :KodeSeksi, TanggalMulai = :TanggalMulai, NoDokumen = :NoDokumen, Keterangan = :Keterangan, Kategori = :Kategori, TglCreate = :TglCreate,  UserId = :UserId,Status = :Status";
                    $exc = $koneksi->prepare($sql);
                    $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                    $exc->bindParam('KodeBranch', $data['KodeBranch'], PDO::PARAM_STR);
                    $exc->bindParam('KodeCabang', $data['KodeCabang'], PDO::PARAM_STR);
                    ...
                    $exc->bindParam('TglCreate', $data['TglCreate'], PDO::PARAM_STR);
                    $exc->bindParam('UserId', $data['UserId'], PDO::PARAM_STR);
                    $exc->bindParam('Status', $status, PDO::PARAM_STR);
                    $exc->execute();
                    $msg['pesan'] = "Berhasil menambah data SK PKWT";
                    $rMsg = "Berhasil menambah data SK PKWT dengan no ktp <b>".$data['NoKtp']."</b>";
                    $msg['status'] = "sukses";
