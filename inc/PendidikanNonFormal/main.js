...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ['NoKtp', 'KodeMaster', 'NoSertifikat', 'Dari'];
	var KetiForm = ["No KTP belum lengkap", "Tingkat belum lengkap", "No Sertifikat belum lengkap", "Tahun Mulai belum lengkap"];
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "" || $("#" + iForm[i]).val() == null) { 
				...
				Customerror("Pendidikan Non Formal", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; 
			}
		}
	}
	console.log($("input[name='kategori']:checked").val());
	if (aksi != "delete") {
		if($("input[name='kategori']:checked").val() == "periode"){
			if($("#Sampai").val() == "" || $("#Sampai").val() == null){
				Customerror("Pendidikan Non Formal", "007", "Berakhir belum lengkap", 'ProsesCrud'); $("#Sampai" ).focus(); scrolltop(); return false; 
			}
		}
	}
